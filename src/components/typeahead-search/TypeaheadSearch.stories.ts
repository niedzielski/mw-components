import Vue from 'vue';
import MwTypeaheadSearch from './TypeaheadSearch.vue';
import { actionSearchClient } from './http/actionSearchClient';

export default {
	title: 'Components/TypeaheadSearch',
	component: MwTypeaheadSearch,
	parameters: { layout: 'centered', actions: { disable: true }, knobs: { disable: true } }
};

export const TypeaheadSearch = (): Vue.Component => Vue.extend( {
	components: { MwTypeaheadSearch },
	data: () => ( { client: actionSearchClient( true ) } ),
	template: `
		<div>
			<mw-typeahead-search :client="client" />
			<p>
